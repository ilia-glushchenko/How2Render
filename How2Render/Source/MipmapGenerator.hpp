...
inline RGBQUAD *GetTexel(MipLevel& mipLevel, uint32_t col, uint32_t row)
{
...
bool GenerateMipmap(Image& image)
{
	if (!image.pixels)
		return false;
	...
	MipLevel& firstMip = image.mipChain.front();
	memcpy(mipPixels, image.pixels, firstMip.byteSize);
	firstMip.data = mipPixels;

	for (size_t i = 1; i < image.mipChain.size(); ++i)
	{
		const MipLevel& currMip = image.mipChain[i - 1];
		MipLevel& nextMip = image.mipChain[i];
