...
inline Shaders CreateShaders(Context &context)
{
	...
	auto hr = CompileShaderFromFile(L"Shaders/PathTracer/PathTracer.fx", "VS", "vs_4_0", &pVSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
	...
	hr = CompileShaderFromFile(L"Shaders/PathTracer/PathTracer.fx", "PS", "ps_4_0", &pPSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
				   L"The FX file cannot be compiled. Please run this executable "
